        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 flex items-center gap-8">
            ...
            <div>
                ...
                <div className="flex items-baseline gap-2 mb-2">
                    ...
                    <span className="text-lg font-semibold" style={{ color: gaugeColor }}>{scoreLabel}</span>
                </div>
        ...
        >
            <div className={`${colorSet.bg} px-4 pt-4 pb-3`}>
                <div className="flex items-start justify-between mb-2">
                    <p className="text-xs text-slate-500 font-semibold uppercase tracking-wide">{kpiData.label}</p>
                    <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${colorSet.badge} capitalize`}>{kpiData.status}</span>
                </div>
                <div className="flex items-baseline gap-1">
                    <span className={`text-3xl font-black tabular-nums ${colorSet.value}`}>{kpiData.value}</span>
...
function AnomalyCard({ anomalyData, cardIndex }) {
    ...
    const severityStyle = anomalyData.severity === 'critical' ? 'border-red-200 bg-red-50' : 'border-amber-200 bg-amber-50'
    const badgeStyle = anomalyData.severity === 'critical' ? 'bg-red-100 text-red-700 border-red-200' : 'bg-amber-100 text-amber-700 border-amber-200'
    const iconColor = anomalyData.severity === 'critical' ? 'text-red-500' : 'text-amber-500'

    return (
        <div className={`rounded-2xl border ${severityStyle} p-4 animate-slide-up`} style={{ animationDelay: `${cardIndex * 0.06}s` }}>
            <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1 flex-wrap">
                        ...
                        <span className={`text-xs font-bold px-2 py-0.5 rounded-full border capitalize ${badgeStyle}`}>{anomalyData.severity}</span>
                    </div>
                ...
                <div className="text-right shrink-0">
                    ...
                    <p className="text-xs text-slate-400">+{anomalyData.deviation_pct}% vs baseline</p>
                </div>
            </div>
        </div>
    )
        ...
        >
            ...
            <div className="flex-1">
                ...
                <div className="w-full h-2 bg-slate-100 rounded-full overflow-hidden">
                    <div className={`h-full rounded-full transition-all duration-700 ${occupancyBarColor}`} style={{ width: `${deptData.bed_occupancy_pct}%` }} />
                </div>
                <p className="text-xs text-slate-400 mt-0.5">{deptData.occupied_beds}/{deptData.total_beds} occupied</p>
            </div>
            <div className="w-24 text-center shrink-0">
                ...
                <p className={`text-lg font-black tabular-nums ${deptData.wait_delta_pct >= 30 ? 'text-red-600' : deptData.wait_delta_pct >= 15 ? 'text-amber-600' : 'text-slate-700'}`}>
                    {deptData.opd_wait_time_min}<span className="text-xs font-normal text-slate-400 ml-0.5">min</span>
                </p>
            </div>
                ...
                {deptData.anomalies?.length > 0 ? (
                    <div className="space-y-1">
                        {deptData.anomalies.map((anomalyItem, anomalyIdx) => (
                            <span key={anomalyIdx} className={`block text-xs font-bold px-2 py-0.5 rounded-full text-center ${anomalyItem.severity === 'critical' ? 'bg-red-100 text-red-700' : 'bg-amber-100 text-amber-700'}`}>
                                {anomalyItem.severity}
                            </span>
                        ))}
                    </div>
                ) : (
                    <span className="block text-xs font-bold px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-700 text-center">Normal</span>
                )}
            </div>
        </div>
    )
...
function InsightCard({ insightData, cardIndex }) {
    ...
    return (
        <div className={`bg-white rounded-2xl border ${priorityStyle.border} shadow-sm p-5 animate-slide-up`} style={{ animationDelay: `${cardIndex * 0.07}s` }}>
            <div className="flex items-start justify-between gap-3 mb-3">
                <div className="flex items-center gap-2">
                    <span className={`w-2 h-2 rounded-full ${priorityStyle.dot} shrink-0`} />
                    <h3 className="font-bold text-slate-800 text-sm leading-snug">{insightData.title}</h3>
                </div>
                <span className={`text-xs font-bold px-2.5 py-1 rounded-full shrink-0 capitalize ${priorityStyle.badge}`}>{insightData.priority}</span>
            </div>
            ...
            <div className="flex items-center justify-between mt-3">
                <span className="text-xs text-slate-400 capitalize">{insightData.department} · {insightData.category}</span>
                <span className="text-xs font-bold text-slate-600">Impact <span className="text-sky-600">{insightData.impact_score}/10</span></span>
            </div>
        </div>
    )
}
...
function RecommendationCard({ recData, cardIndex }) {
    ...
    return (
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 flex items-start gap-4 animate-slide-up" style={{ animationDelay: `${cardIndex * 0.06}s` }}>
            <div className="flex-1">
                <div className="flex items-center gap-2 mb-1 flex-wrap">
                    <h3 className="font-bold text-slate-800 text-sm">{recData.title}</h3>
                    <span className={`text-xs font-bold px-2 py-0.5 rounded-full border capitalize ${urgencyStyle}`}>{recData.urgency.replace('_', ' ')}</span>
                </div>
...
export default function AdminDashboard({ onLogout }) {
    ...
    const loadAllDashboardData = async () => {
        try {
            setFetchError(null)
            const [summaryRes, deptsRes, kpisRes, anomaliesRes, insightsRes, recommendRes, forecastRes] = await Promise.all([
    ...
    // ── Scroll spy — runs after data loads and DOM is painted ──
useEffect(() => {
  ...
  const handleScroll = () => {
    ...
    for (const { sectionId } of SIDEBAR_SECTIONS) {
      ...
      if (top <= NAVBAR_HEIGHT + OFFSET) {
        active = sectionId
      }
    }

    setActiveSidebarItem(active)
  }
...
const scrollToSection = (sectionId) => {
  ...
  setActiveSidebarItem(sectionId)
    ...
    if (fetchError) return (
        <div className="min-h-screen bg-slate-50 flex items-center justify-center px-6">
            <div className="bg-white rounded-2xl border border-red-200 p-8 max-w-md w-full text-center">
                ...
                <button onClick={loadAllDashboardData} className="px-5 py-2.5 bg-sky-500 text-white rounded-xl text-sm font-semibold hover:bg-sky-600 transition-colors">
                    Retry
                </button>
            </div>
        </div>
    ...
    // ── Render ─────────────────────────────────────────────────
    return (
        // ⚠️  NO overflow-hidden here — the window must be able to scroll
        <div className="min-h-screen bg-slate-50 flex flex-col">
            ...
            <header className="bg-white border-b border-slate-200 sticky top-0 z-40 shadow-sm">
                <div className="max-w-screen-2xl mx-auto px-6 h-14 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <button onClick={loadAllDashboardData} className="p-2 rounded-xl border border-slate-200 text-slate-400 hover:text-sky-500 hover:bg-slate-50 transition-all">
                            <RefreshCw size={14} />
                        </button>
            ...
            {/*
                IMPORTANT: This wrapper must NOT have overflow-hidden or overflow-y-auto.
                The window itself must scroll so our window scroll listener fires.
            */}
            <div className="flex flex-1 max-w-screen-2xl mx-auto w-full">

                {/* ── Sidebar — sticky, scrolls independently if nav overflows ── */}
                ...
                {/* ── Main Content — NO overflow-y-auto, NO fixed height ── */}
                <main className="flex-1 px-6 py-6">
                    ...
                    <DashboardSection sectionId="kpis" title="Key Performance Indicators" subtitle="7-day trends with baseline comparison">
                    ...
                    <DashboardSection sectionId="anomalies" title="Active Anomalies" subtitle="Rule-based detection across all departments" anomalyCount={anomalyList.length}>
                        {anomalyList.length === 0 ? (
                            ...
                            <div className="space-y-3">
                                {anomalyList.map((anomalyItem, anomalyIndex) => (
                                    <AnomalyCard key={anomalyItem.anomaly_id} anomalyData={anomalyItem} cardIndex={anomalyIndex} />
                                ))}
                            </div>
                        )}
                    </DashboardSection>
                    ...
                    <DashboardSection sectionId="departments" title="Department Overview" subtitle="All 6 departments — live metrics">
                        <div className="space-y-3">
                            {departmentList.map((deptItem, deptIndex) => (
                                <DepartmentRow key={deptItem.id} deptData={deptItem} rowIndex={deptIndex} />
                            ))}
                        </div>
                    </DashboardSection>

                    <DashboardSection sectionId="insights" title="AI Insights" subtitle="Groq-powered analysis of live hospital data">
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                            {insightList.map((insightItem, insightIndex) => (
                                <InsightCard key={insightItem.insight_id} insightData={insightItem} cardIndex={insightIndex} />
                            ))}
                        </div>
                    </DashboardSection>

                    <DashboardSection sectionId="recommendations" title="Recommendations" subtitle="Actionable items from anomaly analysis">
                        <div className="space-y-3">
                            {recommendList.map((recItem, recIndex) => (
                                <RecommendationCard key={recItem.rec_id} recData={recItem} cardIndex={recIndex} />
                            ))}
                        </div>
                    </DashboardSection>
                    ...
                    <DashboardSection sectionId="forecast" title="48-Hour Forecast" subtitle="Trend-based projection in 6-hour intervals">
                        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
                            <p className="text-sm font-semibold text-slate-600 mb-4">Bed Occupancy Forecast (%)</p>
                            ...
                            <p className="text-sm font-semibold text-slate-600 mt-6 mb-4">OPD Wait Time Forecast (min)</p>
                    ...
                    <DashboardSection sectionId="finance" title="Finance Overview" subtitle="Today's revenue and billing summary">
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                            <FinanceTile tileLabel="Revenue Today" tileValue={`₹${financeData?.revenue_today_lakh}L`} tileSubtext="Across all departments" colorClass="text-emerald-600" />
                            <FinanceTile tileLabel="Revenue MTD" tileValue={`₹${financeData?.revenue_mtd_lakh}L`} tileSubtext={`Target ₹${financeData?.revenue_target_lakh}L`} colorClass="text-sky-600" />
                            <FinanceTile tileLabel="Cost Per Patient" tileValue={`₹${financeData?.cost_per_patient?.toLocaleString()}`} tileSubtext="Average across OPD" colorClass="text-slate-700" />
                            <FinanceTile tileLabel="Pending Bills" tileValue={`₹${financeData?.pending_bills_lakh}L`} tileSubtext="Awaiting settlement" colorClass="text-amber-600" />
                        </div>
                    </DashboardSection>
